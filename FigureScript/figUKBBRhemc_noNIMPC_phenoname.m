...
grid on
    ...
    set(gca,'XTickLabel', []);
ylim([-0.01 0.01])
ylabel('$\hat{h^2}_{NIM}$','interpreter','latex');
...
xlim([0.5, numGroup+0.5])
ylim([-0.01 0.01])
ylabel('$\hat{h^2}$)','interpreter','latex');
box on
grid on
set(gca,'XTick',[1:numGroup]);
set(gca,'XTickLabel', []);
...
set(gcf,'PaperPosition',[0 0 20 20])
  ylabel('$\hat{\Delta}_{h^2}$','interpreter','latex');
saveas(1,'figUKBBRhemcnoNIMPC_phenoname.png');
